...
def ListToNum(list):
    """Return the dotted RCS number whose elements are those in the list."""
...
def StringToDate(s):
    ...
    m = re.search(r'^(\d\d|\d\d\d\d).(\d\d).(\d\d).(\d\d).(\d\d).(\d\d)$', s)
...
def DateToString(Y, MM, DD, hh, mm, ss):
    ...
    if not Y >= 1900:
        raise RcsError("Invalid year value in date %s" %
                       [Y, MM, DD, hh, mm, ss])
    if not 1 <= MM <= 12:
        raise RcsError("Invalid month value in date %s" %
                       [Y, MM, DD, hh, mm, ss])
    if not 1 <= DD <= 31:
        raise RcsError("Invalid day value in date %s" %
                       [Y, MM, DD, hh, mm, ss])
    if not 0 <= hh <= 23:
        raise RcsError("Invalid hour value in date %s" %
                       [Y, MM, DD, hh, mm, ss])
    if not 0 <= mm <= 59:
...
def TextFromDiff(source, diff):
    ...
    while True:
        ...
        if re.search(r'^\s*$', s) is not None:
            break
        res = re.search(r'^([ad])([0-9]+)\s+([0-9]+)\s*$', s)
        if res is None:
...
def SymNumStringToList(s):
    r = re.compile(r'^\s*(?:([0-9]*[a-z][0-9a-z]*):([0-9.]+))\s+(.*)$')
    ret = []
    m = re.search(r, s)
    while m is not None:
        ret += (m[0], m[1])
...
class RcsDelta:
    ...
    def textToDiff(self, prev_rev):
        ...
        if prev_rev.getTextIsDiff():
            raise RcsError("previous revision %s is a diff!" %
                           prev_rev.getRevision())
...
def ParseRcs(text):
    ...
    if lex.getKw("branch", False) is not None:
    ...
    if lex.getKw("strict", False) is not None:
        rcs.setStrict("", False)
        lex.getSemicolon()

    if lex.getKw("integrity", False) is not None:
        rcs.setIntegrity(lex.getString(False), False)
        lex.getSemicolon()

    if lex.getKw("comment", False) is not None:
        rcs.setComment(lex.getString(False), False)
        lex.getSemicolon()

    if lex.getKw("expand", False) is not None:
    ...
    while True:
        ...
        if lex.getKw("commitid", False) is not None:
